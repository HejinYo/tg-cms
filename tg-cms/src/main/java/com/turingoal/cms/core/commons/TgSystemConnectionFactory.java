 ...
 * 获取连接池
 */
public final class TgSystemConnectionFactory {
    ...
    private interface Singleton {
        TgSystemConnectionFactory INSTANCE = new TgSystemConnectionFactory();
    }

    private TgSystemConnectionFactory() {
        ...
        // 数据库密码加密
        if (Boolean.valueOf(PropsUtil.getValue("db.decrypt", propFileName))) {
            try {
                ds.setFilters("config");
            } catch (SQLException e) {
                e.printStackTrace();
            }
            ds.setConnectionProperties("config.decrypt=true;config.decrypt.key=" + PropsUtil.getValue("db.publicKey", propFileName));
        }
