 ...
 * 后台验证用户凭证
 */
public class TgSecurityAuthenticationProvider4admin extends SpringSecurityAbstractUserDetailsAuthenticationProvider {
    ...
    public Authentication authenticate(final Authentication authentication) throws AuthenticationException {
        TgSecurityAuthenticationToken4admin authenticationToken = (TgSecurityAuthenticationToken4admin) authentication;
        Authentication auth;
        // 开启验证码功能的情况
    ...
    private void checkUserDomain(final UserDetails userDetail) {
        ...
        // 为空或不等于admin抛出异常
        if (StringUtil.isBlank(userDomain) || !"admin".equals(userDomain)) {
            if (isHideUserNotFoundExceptions()) {
                throw new BadCredentialsException(getMessagesSource().getMessage("AbstractUserDetailsAuthenticationProvider.badCredentials", "认证失败"));
            } else {
    ...
    public boolean supports(final Class<?> authentication) {
        return TgSecurityAuthenticationToken4admin.class.isAssignableFrom(authentication);
    }
