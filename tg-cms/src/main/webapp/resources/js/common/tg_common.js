...
// 基本表单ajax 提交
function tg_baseFormAjaxSubmit(formId, rules, messages, sucCallbackFun,
		failCallbackFun) {
...
// 表单窗口ajax 提交
function tg_formWinAjaxSubmit(formModalId, formId, rules, messages,
		sucCallbackFun, failCallbackFun) {
...
var TG_Tabel = {
	createNew : function(option) {
		...
		tg_table.add = function(windowId, formId, rules, messages) {
			tg_formWinAjaxSubmit(windowId, formId, rules, messages,
					tg_table.refresh);
		};
		// 删除
...
// 排序
function updateOrder(updateOrderUrl, oldIndex, newIndex) {
	var parms = {
		oldIndex : oldIndex,
		newIndex : newIndex
	}
	tg_simpleAjaxPost(updateOrderUrl, parms);
}